class AIAgent {
    ...
    async _executeBattle(levelId) {
        ...
        const allUnlockedSorted = ownedCharacters
            .filter(char => !char.locked)
            .sort((a, b) => b.power - a.power);

        // 2. Créer une liste ne contenant que la version la plus puissante de chaque personnage unique.
        const uniqueBestChars = [];
        const seenNames = new Set();
        for (const char of allUnlockedSorted) {
            if (!seenNames.has(char.name)) {
                uniqueBestChars.push(char);
                seenNames.add(char.name);
            }
        }

        // 3. Sélectionner les 3 meilleurs personnages de cette liste filtrée et unique.
        const bestChars = uniqueBestChars.slice(0, 3);
    ...
    calculateReward(oldState, newState, actionResult) {
        ...
        if (navigationPenaltyActions.includes(actionResult.name)) {
            let penalty;
            if (actionResult.name === "Annuler la sélection") {
                penalty = -5.0; // Pénalité plus forte pour annuler un combat
                addLog("Pénalité (Annulation de combat)", penalty);
            } else {
                penalty = -0.5; // Petite pénalité pour les autres navigations
