class AIAgent {
    ...
    setDashboard(dashboard) {
        ...
        // Monkey patch la fonction de log du dashboard pour utiliser notre version améliorée
        if (this.dashboard && typeof logEnhanced === 'function') {
            this.dashboard.log = (message) => logEnhanced(message, this.dashboard.logContainer);
        }
        if (this.dashboard && typeof updateQValuesEnhanced === 'function') {
            this.dashboard.updateQValues = (qValueLog) => updateQValuesEnhanced(qValueLog, document.getElementById('ai-q-values-container'));
        }
    }
    ...
    async chooseAction(state) {
        ...
        if (Math.random() <= this.explorationRate) {
            ...
            if(this.dashboard.updateQValues) this.dashboard.updateQValues([]); // Clear Q-Values display
            return actionIndex;
        }
        ...
        availableActions.forEach((action, index) => {
            const qValue = availableQValues[index];
            const isChosen = index === actionIndex;
            this.logReflection(`${isChosen ? '-> ' : '   '}[${action.name}]: Q-Value = ${qValue.toFixed(4)}`);
            qValueLog.push({ name: action.name, qValue: qValue, isChosen: isChosen });
        });

        if(this.dashboard.updateQValues) this.dashboard.updateQValues(qValueLog);
